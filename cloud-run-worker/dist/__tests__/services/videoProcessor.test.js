...
describe('compressVideoIfNeeded', () => {
    ...
    describe('Compression for large files', () => {
        it('should compress files over 200MB threshold', async () => {
            ...
            expect(mockExecFileAsync).toHaveBeenCalledWith('ffmpeg', [
                '-i', mockInputPath,
                '-vcodec', 'libx264',
                '-crf', '28',
                '-preset', 'fast',
    ...
    describe('Error handling', () => {
        it('should throw error when ffmpeg fails', async () => {
            ...
            mockExistsSync.mockReturnValueOnce(true);
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toThrow('FFmpeg encoding error: invalid codec');
        ...
        it('should throw error when output file is not created', async () => {
            ...
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toThrow('Compression completed but output file not found');
        ...
        it('should cleanup temporary file on ffmpeg error', async () => {
            ...
            mockExistsSync.mockReturnValueOnce(true);
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toThrow('FFmpeg timeout');
            // Verify cleanup
            expect(mockExistsSync).toHaveBeenCalledWith(mockOutputPath);
            expect(mockUnlinkSync).toHaveBeenCalledWith(mockOutputPath);
        ...
        it('should handle non-Error objects thrown by ffmpeg', async () => {
            ...
            mockExistsSync.mockReturnValueOnce(false);
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toBe('Unknown error string');
            // Verify error was logged (console.error was mocked)
            expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] Compression failed: Unknown error`));
        });
        it('should not cleanup if temporary file does not exist on error', async () => {
            ...
            mockExistsSync.mockReturnValueOnce(false);
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toThrow('FFmpeg crash');
    ...
    describe('File operations', () => {
        it('should replace original file with compressed version', async () => {
            ...
            const renameCall = mockRenameSync.mock.invocationCallOrder[0];
    ...
    describe('Logging behavior', () => {
        it('should log skip message for small files', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] File size 100.0MB is under 200MB threshold, skipping compression`));
        });
        it('should log compression start for large files', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] File size 300.0MB exceeds threshold, starting compression...`));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] Running ffmpeg compression (CRF 28, fast preset)...`));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] Compression completed in`));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] Replaced original file with compressed version`));
        });
        it('should log error message on compression failure', async () => {
            ...
            mockExistsSync.mockReturnValueOnce(true);
            // Execute & Assert
            await expect(compressVideoIfNeeded(mockInputPath, mockUploadId)).rejects.toThrow(errorMessage);
            // Verify error log
            expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`[${mockUploadId}] Compression failed: ${errorMessage}`));
        });
    });
});
//# sourceMappingURL=videoProcessor.test.js.map