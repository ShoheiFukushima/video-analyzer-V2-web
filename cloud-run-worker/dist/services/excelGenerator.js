...
export const generateExcelReport = async (outputPath, fileName, analysis) => {
    // Create workbook
    ...
    const metadataData = [
        ...
        ['Unique Words (Transcription)', new Set(analysis.transcription.flatMap(seg => seg.text.toLowerCase().split(/\s+/))).size],
        ['Unique Words (OCR)', new Set(analysis.ocrResults.flatMap(ocr => ocr.text.toLowerCase().split(/\s+/))).size],
        [''],
        ['Average Confidence'],
        ['Transcription', ((analysis.transcription.reduce((sum, seg) => sum + seg.confidence, 0) / analysis.transcription.length) * 100).toFixed(1) + '%'],
        ['OCR', ((analysis.ocrResults.reduce((sum, ocr) => sum + ocr.confidence, 0) / analysis.ocrResults.length) * 100).toFixed(1) + '%'],
    ];
