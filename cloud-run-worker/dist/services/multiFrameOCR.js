 ...
 */
export async function extractMultipleFrames(videoPath, scene, outputDir, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    const frames = [];
    const duration = scene.endTime - scene.startTime;
    for (const position of fullConfig.positions) {
        const timestamp = scene.startTime + (duration * position);
        const framePath = path.join(outputDir, `scene-${scene.sceneNumber.toString().padStart(4, '0')}-p${(position * 100).toFixed(0)}.png`);
 ...
 * @returns OCR result
 */
async function performSingleFrameOCR(frame, model) {
    try {
 ...
 */
export async function performMultiFrameOCR(scene, frames, model, rateLimiter, config = {}) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    const limit = pLimit(3); // Parallel limit for frames within a scene
    // Process all frames in parallel (within rate limits)
    const results = await Promise.all(frames.map(frame => limit(async () => {
 ...
 */
function selectBestFrame(results, strategy) {
    if (results.length === 0) {
 ...
 */
function selectFirstStableFrame(results) {
 ...
 */
function selectMostTextFrame(results) {
    return results.reduce((best, current) => current.text.length > best.text.length ? current : best);
}
/**
 * Select frame with highest OCR confidence
 */
function selectHighestConfidenceFrame(results) {
 ...
 */
function findTextAppearancePosition(results) {
 ...
 */
function findTextStabilityPosition(results) {
    const normalizeText = (text) => text.trim().toLowerCase().replace(/\s+/g, ' ');
    for (let i = 0; i < results.length - 1; i++) {
        const current = results[i];
        const next = results[i + 1];
 ...
 */
export async function processMultiFrameOCRBatch(scenes, videoPath, outputDir, uploadId) {
    ...
    const processScene = async (scene) => {
        ...
        const multiFrameResult = await performMultiFrameOCR(scene, frames, model, rateLimiter, { strategy: 'first_stable' });
