...
export const uploadResultFile = async (filePath, uploadId) => {
    try {
        ...
        if (process.env.NODE_ENV === 'production') {
            throw new Error(`Failed to upload result file to Vercel Blob: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
...
export const uploadToGCS = async (filePath, uploadId) => {
    try {
        const fileName = `${uploadId}_analysis.xlsx`;
