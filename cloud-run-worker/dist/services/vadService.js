 ...
 */
async function splitIntoChunks(audioPath, segments, outputDir, maxDuration) {
    const chunks = [];
    if (segments.length === 0) {
 ...
 */
export async function extractAudioChunk(audioPath, chunk) {
    await extractChunkWithFFmpeg(audioPath, chunk.filePath, chunk.startTime, chunk.duration);
 ...
 */
export async function cleanupVADFiles(outputDir) {
    try {
