 ...
 */
export async function detectWithPyScene(videoPath, videoDuration, onProgress) {
    ...
    if (videoDuration && videoDuration > 0) {
        ...
        const scaledTimeout = Math.max(config.timeoutMs, Math.ceil((600000 + Math.ceil(videoDuration / 300) * 60000) * dissolveMultiplier));
    ...
    const outputJson = path.join(os.tmpdir(), `pyscene_${Date.now()}_${Math.random().toString(36).slice(2)}.json`);
    ...
    try {
        ...
        const cuts = results.map((r) => ({
            timestamp: Math.floor(r.timestamp * 1000) / 1000, // ミリ秒精度を保持（0.1s丸めによるカット消失を防止）
            confidence: r.confidence,
            ...(r.type === 'dissolve' ? { detectionReason: 'dissolve_transition' } : {}),
 ...
 */
function runPySceneProcess(videoPath, outputJson, config) {
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', async (code) => {
            ...
            try {
                ...
                const results = Array.isArray(parsed) ? parsed : parsed.cuts;
                const telopAnimations = Array.isArray(parsed) ? [] : (parsed.telop_animations || []);
                const panAnimations = Array.isArray(parsed) ? [] : (parsed.pan_animations || []);
...
//# sourceMappingURL=pysceneDetector.js.map