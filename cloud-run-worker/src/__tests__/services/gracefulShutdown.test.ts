...
describe('Graceful Shutdown Error Messages', () => {
  describe('Error Message Classification', () => {
    const getErrorMessage = (signal: string): { message: string; code: string } => {
      switch (signal) {
        case 'SIGTERM':
          return {
  ...
  describe('Frontend Error Display Classification', () => {
    const isServerInterruption = (error: string | null): boolean => {
  ...
  describe('Error Message User Experience', () => {
    it('should provide actionable guidance for all error types', () => {
      ...
      errorMessages.forEach(message => {
        ...
        expect(
          message.includes('try uploading again') ||
          message.includes('try with a smaller video')
        ).toBe(true);
      });
    });
