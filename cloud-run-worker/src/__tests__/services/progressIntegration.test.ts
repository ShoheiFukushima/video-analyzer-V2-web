...
import type { ProgressCallback, ProgressInfo } from '../../types/progress.js';
import {
...
describe('Progress Integration', () => {
  ...
  describe('StatusManager Integration', () => {
    it('should create callback that updates StatusManager', async () => {
      ...
      const callback = createStatusUpdateCallback(
        'test-upload',
        mockUpdatePhaseProgress as unknown as (
          uploadId: string,
          phase: 1 | 2 | 3,
          phaseProgress: number,
          options?: { subTask?: string; stage?: string }
        ) => Promise<unknown>
      );
      ...
      expect(mockUpdatePhaseProgress).toHaveBeenCalledWith(
        'test-upload',
        2, // Phase 2 for frame extraction
    ...
    it('should calculate correct phase progress for frame extraction', async () => {
      ...
      expect(mockUpdatePhaseProgress).toHaveBeenLastCalledWith(
        'test-upload',
        2,
        25, // Start of frame extraction range
        expect.anything()
      );
      ...
      expect(mockUpdatePhaseProgress).toHaveBeenLastCalledWith(
        'test-upload',
        2,
        50, // End of frame extraction range
        expect.anything()
      );
    });
