...
const SEVERITY_LEVELS: Record<AlertPayload['severity'], number> = {
  info: 0,
  warning: 1,
  error: 2,
  critical: 3,
...
export function loadAlertConfig(): AlertConfig {
  return {
    ...
    minSeverity: (process.env.ALERT_MIN_SEVERITY as AlertPayload['severity']) || 'error',
  };
}
 ...
 */
function formatPlainTextAlert(payload: AlertPayload, context: AlertContext, config: AlertConfig): string {
  ...
  if (context.uploadId) lines.push(`Upload ID: ${context.uploadId}`);
 ...
 */
function formatHtmlAlert(payload: AlertPayload, context: AlertContext, config: AlertConfig): string {
  const timestamp = new Date().toISOString();
  const severityColor = {
    info: '#2196F3',
    warning: '#FF9800',
 ...
 */
function logAlertToConsole(payload: AlertPayload, context: AlertContext, config: AlertConfig): void {
 ...
 */
export async function sendTransNetFailureAlert(
  error: string,
  context: AlertContext = {}
): Promise<boolean> {
  return sendAlert(
 ...
 */
export async function sendProcessingSuccessAlert(
  stats: { totalScenes: number; processingTimeMs: number },
  context: AlertContext = {}
): Promise<boolean> {
  return sendAlert(
    {
 ...
 */
function shouldThrottleAlert(alertKey: string): boolean {
  const now = Date.now();
 ...
 */
export function clearAlertThrottle(): void {
  recentAlerts.clear();
}
