...
const DEFAULT_CONFIG: TextStabilityConfig = {
 ...
 */
export async function extractStabilizationFrames(
  videoPath: string,
  stabilizationPoint: StabilizationPoint,
  outputDir: string,
  config: Partial<TextStabilityConfig> = {}
  ...
  for (let i = 0; i < fullConfig.framesToCheck; i++) {
    ...
    const framePath = path.join(
      outputDir,
      `stab-${stabilizationPoint.timestamp.toFixed(2)}-f${i}.png`
    );
 ...
 */
export async function performFrameOCR(
 ...
 */
export function findStableTextFrame(
  ocrResults: FrameOCRResult[]
): FrameOCRResult | null {
  if (ocrResults.length < 2) return null;
  ...
  const frameWithMostText = ocrResults.reduce((best, current) =>
 ...
 */
export async function detectTextStabilization(
  videoPath: string,
  stabilizationPoint: StabilizationPoint,
  outputDir: string,
  model: GenerativeModel,
  ...
  const frames = await extractStabilizationFrames(
    videoPath,
    stabilizationPoint,
    outputDir,
    config
  ...
  const matchingFrames = ocrResults.filter(r =>
    r.text.trim().toLowerCase() === normalizedText
  ).length;
 ...
 */
export async function processStabilizationPoints(
  videoPath: string,
  ...
  const results: StableTextResult[] = [];

  for (const point of stabilizationPoints) {
    const result = await detectTextStabilization(
      videoPath,
      point,
      outputDir,
      model,
