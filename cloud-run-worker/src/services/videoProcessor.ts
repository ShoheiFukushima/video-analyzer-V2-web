...
export const processVideo = async (
  ...
  try {
    ...
    try {
      ...
      // Helper to check if we should run a step based on checkpoint
      type CheckpointStep = typeof cp.currentStep;
      const stepOrder: CheckpointStep[] = [
        'downloading', 'audio_extraction', 'transcription', 'scene_detection', 'ocr', 'excel_generation'
      ];
      const getStepIndex = (step: CheckpointStep) => stepOrder.indexOf(step);
      const shouldRunStep = (step: CheckpointStep) => getStepIndex(cp.currentStep) <= getStepIndex(step);
 ...
 */
async function performTranscriptionWithCheckpoint(
  uploadId: string,
  audioPath: string,
  hasAudio: boolean,
