...
export interface OCRRouterConfig {
  ...
  autoParallelBoostThreshold: number; // Duration in seconds (default: 3600 = 1 hour)
...
const DEFAULT_CONFIG: OCRRouterConfig = {
...
export class OCRRouter {
  private readonly config: OCRRouterConfig;
  ...
  setVideoDuration(durationSeconds: number): void {
    ...
    if (this.isLongVideo) {
      console.log(
        `[OCRRouter] Long video detected (${(durationSeconds / 60).toFixed(1)} min). ` +
   ...
   */
  private getEffectiveParallelLimit(): number {
   ...
   */
  private getAvailableProviders(): OCRProvider[] {
   ...
   */
  private selectProvider(): OCRProvider | null {
   ...
   */
  private selectRoundRobin(available: OCRProvider[]): OCRProvider {
   ...
   */
  private selectLoadBalanced(available: OCRProvider[]): OCRProvider {
   ...
   */
  async processWithFallback(imageBuffer: Buffer): Promise<OCRResult> {
    const errors: Error[] = [];
   ...
   */
  async processParallel(tasks: ImageTask[]): Promise<BatchOCRResult> {
    const startTime = Date.now();
    const effectiveParallel = this.getEffectiveParallelLimit();
    const limit = pLimit(effectiveParallel);

    console.log(
      `[OCRRouter] Processing ${tasks.length} images in parallel ` +
    ...
    // Log summary
    console.log(
      `[OCRRouter] Batch complete: ${successCount}/${tasks.length} succeeded ` +
  ...
  getAllProviderStats(): Record<string, OCRProviderStats> {
    const stats: Record<string, OCRProviderStats> = {};
  ...
  logStatus(): void {
    ...
    for (const [name, stat] of Object.entries(stats)) {
      console.log(
        `  [${name}] requests=${stat.totalRequests}, ` +
...
let routerInstance: OCRRouter | null = null;

/**
 * Get or create the OCR router singleton
 */
export function getOCRRouter(config?: Partial<OCRRouterConfig>): OCRRouter {
  if (!routerInstance) {
 ...
 */
export function resetOCRRouter(): void {
  routerInstance = null;
}
...
export async function performBatchOCR(
  ...
  const tasks: ImageTask[] = images.map((buffer, index) => ({
    id: index,
    imageBuffer: buffer,
  }));
