...
interface LLMCleansingConfig {
  enabled: boolean;
  model: string;
  apiKey: string;
  chunkSize: number;       // scenes per LLM request
  maxRetries: number;
  initialDelayMs: number;  // for exponential backoff
}
...
function loadConfig(): LLMCleansingConfig {
  return {
 ...
 */
export async function cleanseScenesWithLLM(
  scenes: SceneTextData[]
): Promise<CleansedResult[]> {
  ...
  const allResults: CleansedResult[] = [];
  ...
  for (let i = 0; i < scenes.length; i++) {
    if (allResults[i].ocr !== scenes[i].ocr) modifiedOcr++;
    if (allResults[i].narration !== scenes[i].narration) modifiedNarration++;
  }
...
async function callGemini(
  ...
  if (!Array.isArray(parsed) || parsed.length !== chunk.length) {
    throw new Error(
      `Response length mismatch: expected ${chunk.length}, got ${Array.isArray(parsed) ? parsed.length : 'non-array'}`
    );
  }
