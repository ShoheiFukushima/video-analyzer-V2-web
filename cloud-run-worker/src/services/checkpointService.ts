...
import {
  CHECKPOINT_EXPIRATION_DAYS,
  CURRENT_BUILD_COMMIT,
  createInitialCheckpoint,
  generateIntermediatePaths,
  isCheckpointExpired,
} from '../types/checkpoint.js';
...
const inMemoryCheckpoints = new Map<string, ProcessingCheckpoint>();

/**
 * Load checkpoint from Turso (or in-memory for development)
 */
export async function loadCheckpoint(uploadId: string): Promise<ProcessingCheckpoint | null> {
...
export async function updateCheckpoint(
  ...
  const updated: ProcessingCheckpoint = {
    ...existing,
    ...update,
 ...
 */
export async function deleteCheckpoint(uploadId: string): Promise<void> {
 ...
 */
export async function updateCheckpointStep(
  uploadId: string,
  step: CheckpointStep
): Promise<void> {
 ...
 */
export async function addCompletedAudioChunks(
  uploadId: string,
  chunkIndices: number[],
 ...
 */
export async function setSceneCuts(
  uploadId: string,
  sceneCuts: SceneCut[]
): Promise<void> {
  await updateCheckpoint(uploadId, {
 ...
 */
export async function addCompletedOcrScenes(
  uploadId: string,
  sceneIndices: number[],
 ...
 */
export function getIntermediatePaths(userId: string, uploadId: string): IntermediateFilePaths {
...
export async function cleanupExpiredCheckpoints(): Promise<number> {
  ...
  try {
    ...
    for (const row of expired.rows) {
      ...
      const videoPath = row.intermediate_video_path as string | null;
      const audioPath = row.intermediate_audio_path as string | null;

      try {
        if (videoPath) await deleteFromR2(videoPath);
 ...
 */
export async function canResumeFromCheckpoint(
  uploadId: string,
  userId: string
): Promise<{ canResume: boolean; step?: CheckpointStep; message?: string }> {
  const checkpoint = await loadCheckpoint(uploadId);
 ...
 */
function mapRowToCheckpoint(row: Record<string, unknown>): ProcessingCheckpoint {
  return {
    uploadId: row.upload_id as string,
    userId: row.user_id as string,
    currentStep: row.current_step as CheckpointStep,
    buildCommit: (row.build_commit as string | null) ?? undefined,
    intermediateVideoPath: row.intermediate_video_path as string | undefined,
    intermediateAudioPath: row.intermediate_audio_path as string | undefined,
    videoDuration: row.video_duration as number | undefined,
    totalAudioChunks: row.total_audio_chunks as number | undefined,
    totalScenes: row.total_scenes as number | undefined,
    completedAudioChunks: JSON.parse(row.completed_audio_chunks as string || '[]'),
    transcriptionSegments: JSON.parse(row.transcription_segments as string || '[]'),
    sceneCuts: JSON.parse(row.scene_cuts as string || '[]'),
    completedOcrScenes: JSON.parse(row.completed_ocr_scenes as string || '[]'),
    ocrResults: JSON.parse(row.ocr_results as string || '{}'),
