...
export function createProgressTracker(config: ProgressTrackerConfig): ProgressTracker {
   ...
   */
  const callCallback = (forceCall: boolean = false): void => {
   ...
   * Increment progress
   */
  const incrementProgress = (itemLabel?: string): void => {
   ...
   * Update total items
   */
  const setTotalItems = (newTotal: number): void => {
...
export function createStatusUpdateCallback(
  ...
  return async (progress: ProgressInfo) => {
    ...
    const subTask =
      progress.phase === 'frame_extraction'
