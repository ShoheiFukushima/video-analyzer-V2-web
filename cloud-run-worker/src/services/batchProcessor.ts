...
import { loadCheckpoint, saveCheckpoint, addCompletedOcrScenes } from './checkpointService.js';
...
import type { Scene, SceneCut } from '../types/excel.js';

/**
 * Scene with OCR result
 */
 ...
 */
export async function processSingleBatch(payload: BatchPayload): Promise<BatchResult> {
  const {
    uploadId,
    batchIndex,
    totalBatches,
  ...
  const scenes = checkpoint.sceneCuts as unknown as Scene[];
  ...
  try {
    ...
    const scenesWithFrames = await extractFramesForScenes(
      localVideoPath,
      scenesToProcess,
      framesDir,
      startSceneIndex
    ...
    const ocrResults = await runOcrOnScenes(
      uploadId,
      scenesWithFrames,
      videoDuration,
      batchNumber,
 ...
 */
async function extractFramesForScenes(
  videoPath: string,
  scenes: Scene[],
  framesDir: string,
  globalIndexOffset: number
): Promise<Array<Scene & { framePath: string; globalIndex: number }>> {
  const results: Array<Scene & { framePath: string; globalIndex: number }> = [];
 ...
 */
async function runOcrOnScenes(
  uploadId: string,
  scenes: Array<Scene & { framePath: string; globalIndex: number }>,
  videoDuration: number,
  batchNumber: number,
  totalBatches: number
  ...
  const sceneIndexMap: Map<number, number> = new Map(); // task id -> globalIndex
