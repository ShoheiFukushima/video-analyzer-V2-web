...
import { processAudioWithVAD, extractAudioChunk, cleanupVADFiles, type AudioChunk, type VADResult, type VoiceSegment } from './vadService.js';
import { splitAudioIntoChunks, cleanupPreChunks, getAudioMetadata, type AudioFileChunk } from './audioExtractor.js';
 ...
 * ```
 */
export async function processAudioWithVADAndWhisper(
  audioPath: string,
  uploadId: string,
  ...
  try {
    ...
    if (vadResult.audioChunks.length === 0) {
      ...
      const fallbackChunks = await createFallbackChunks(
        audioPath,
        vadResult.totalDuration,
        chunksDir,
        uploadId
      );
      ...
      const stats = calculateFallbackStatistics(
        vadResult,
    ...
    const segments = await transcribeChunksWithWhisper(
      vadResult.audioChunks,
      uploadId,
      5, // concurrency
      checkpoint
    );
 ...
 */
async function extractAllAudioChunks(
  audioPath: string,
  chunks: AudioChunk[]
): Promise<void> {
 ...
 */
async function runVADDetection(
  audioPath: string,
  chunksDir: string,
  uploadId: string
): Promise<VADResult> {
  ...
  try {
    ...
    const preChunks = await splitAudioIntoChunks(
      audioPath,
      preChunksDir,
      audioDuration,
      DEFAULT_PRE_CHUNK_CONFIG
    );
    ...
    const vadResults: { result: VADResult; offset: number }[] = [];
...
function mergeVADResults(
  vadResults: { result: VADResult; offset: number }[],
  totalDuration: number,
  uploadId: string
): VADResult {
  console.log(`[${uploadId}] Merging ${vadResults.length} VAD results...`);

  const allVoiceSegments: VoiceSegment[] = [];
  const allAudioChunks: AudioChunk[] = [];
 ...
 */
function deduplicateSegments(segments: VoiceSegment[], threshold: number): VoiceSegment[] {
  if (segments.length === 0) return [];

  // Sort by start time
  const sorted = [...segments].sort((a, b) => a.startTime - b.startTime);

  const result: VoiceSegment[] = [sorted[0]];
 ...
 */
function deduplicateChunks(chunks: AudioChunk[], threshold: number): AudioChunk[] {
  if (chunks.length === 0) return [];

  // Sort by start time
  const sorted = [...chunks].sort((a, b) => a.startTime - b.startTime);

  const result: AudioChunk[] = [sorted[0]];
 ...
 */
async function transcribeChunksWithWhisper(
  chunks: AudioChunk[],
  uploadId: string,
 ...
 */
function calculatePipelineStatistics(
  vadResult: VADResult,
  totalWhisperCalls: number
): { vadStats: PipelineResult['vadStats'], whisperStats: PipelineResult['whisperStats'] } {
 ...
 */
function buildEmptyResult(vadResult: VADResult): PipelineResult {
  return {
    segments: [],
    vadStats: {
      totalDuration: vadResult.totalDuration,
 ...
 */
function logPipelineCompletion(
  uploadId: string,
  stats: { vadStats: PipelineResult['vadStats'], whisperStats: PipelineResult['whisperStats'] }
): void {
...
/**
 * Whisper API response format (verbose JSON)
 */
...
/**
 * Whisper API request parameters
 */
interface WhisperRequestParams {
  file: File;
  model: 'whisper-1';
  language: string;
 ...
 */
function validateChunkFile(chunk: AudioChunk, uploadId: string): boolean {
 ...
 */
function prepareWhisperRequest(chunk: AudioChunk): WhisperRequestParams {
...
function parseWhisperResponse(
  responseData: WhisperVerboseResponse,
  chunk: AudioChunk
 ...
 */
async function createFallbackChunks(
  audioPath: string,
  totalDuration: number,
  chunksDir: string,
  uploadId: string
 ...
 */
function logFallbackCompletion(
  uploadId: string,
  stats: { vadStats: PipelineResult['vadStats'], whisperStats: PipelineResult['whisperStats'] }
): void {
  console.log(`[${uploadId}] ═══════════════════════════════════════`);
