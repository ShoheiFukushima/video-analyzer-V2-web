...
export interface ParallelDownloadOptions {
  chunkSize?: number;      // Default: auto-adjusted based on file size
  concurrency?: number;    // Default: auto-adjusted based on file size
  onProgress?: (downloaded: number, total: number) => void;
  maxRetries?: number;     // Default: 5 (increased for large file resilience)
}
 ...
 */
function getDownloadSettings(fileSize: number): { chunkSize: number; concurrency: number; description: string } {
 ...
 */
export async function downloadFromR2Parallel(
  key: string,
  destPath: string,
