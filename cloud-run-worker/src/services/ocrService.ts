...
export const extractFramesAndOCR = async (
  videoPath: string,
  uploadId: string
): Promise<OCRResult[]> => {
  ...
  try {
    ...
    try {
      ...
      await execAsync(
        `ffmpeg -i "${videoPath}" -vf fps=1/${frameInterval} "${framesDir}/frame_%04d.jpg" -y 2>/dev/null`,
...
function generateMockOCRResults(): OCRResult[] {
  return [
    {
      timestamp: 0.5,
      frameIndex: 0,
